...
public class Player : MonoBehaviour
{
    ...
    void GetInput()
    {
        ...
        if (Input.GetKey(GameManager.i.keyJump) && onGround) MoveJump();

        //for some reason it's working with X but not with keyJump?
        //remember to bring up the (funny) physics bug
        bool jumpReleased = Input.GetKey(KeyCode.X); //Input.GetKeyUp(GameManager.i.keyJump);
